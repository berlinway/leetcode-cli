...
const {request} = require('http2-request');
var URL = require('url');
...
plugin.getCategoryProblems = function(category, cb) {
  ...
  const opts = plugin.makeOpts(config.sys.urls.base,config.sys.urls.problems.replace('$category', category));
...
plugin.getProblem = function(problem, needTranslation, cb) {
  ...
  const opts = plugin.makeOpts(config.sys.urls.base,config.sys.urls.graphql);
  opts.method = 'POST';
...
plugin.testProblem = function(problem, cb) {
  ...
  const opts = plugin.makeOpts(config.sys.urls.base,config.sys.urls.test.replace('$slug', problem.slug));
...
plugin.submitProblem = function(problem, cb) {
  ...
  const opts = plugin.makeOpts(config.sys.urls.base,config.sys.urls.submit.replace('$slug', problem.slug));
...
plugin.getSubmission = function(submission, cb) {
  ...
  const opts = plugin.makeOpts(config.sys.urls.base,config.sys.urls.submission.replace('$id', submission.id));
...
plugin.getFavorites = function(cb) {
  ...
  const opts = plugin.makeOpts(config.sys.urls.base,config.sys.urls.favorites);
...
function runSession(method, data, cb) {
  const opts = plugin.makeOpts(config.sys.urls.base,config.sys.urls.session);
  opts.json = true;
  opts.method = method;
  opts.body = data;
...
plugin.signin = function(user, cb) {
  ...
  request(config.sys.urls.login, function(e, resp, body) {
    ...
    var tmpurl = URL.parse(config.sys.urls.login);
    const opts = {
      method:'POST',
      url:     config.sys.urls.base,
      headers: {
        ":path": tmpurl.path,
